...
let tradeBy20Prices = trade => {
    ...
    if (currentAvPrice - prevAvPrice >= 3 && !canISell && trendSignal) {
        try {
            ...
            fs.appendFile(
                'message.txt',
                `Buy: ${buyPrice}; Date:${moment().format(
        ...
        try {
            const profit =
                trade[trade.length - 1] / buyPrice > 1
                    ...
                    : Number(
                          -1 *
                              (100 - (trade[trade.length - 1] / buyPrice) * 100)
                      );
            totalProfit += profit;
            fs.appendFile(
                'message.txt',
                `Sell: ${trade[trade.length - 1]}; Date:${moment().format(
                    'MMMM Do YYYY, h:mm:ss a'
...
let tradeByCurrAndPrevPrices = trade => {
    ...
    if (currentPrice - prevPrice >= 1 && !canISell) {
        try {
            fs.appendFile(
                'message.txt',
                `Buy: ${currentPrice}; Date:${moment().format(
                    'MMMM Do YYYY, h:mm:ss a'
                )}\n`,
                err => {
                    if (err) throw err;
    ...
    if (prevPrice - currentPrice >= 1 && canISell && buysCounter !== 0) {
        try {
            ...
            fs.appendFile(
                'message.txt',
                `Sell: ${currentPrice}; Date:${moment().format(
                    'MMMM Do YYYY, h:mm:ss a'
                )}\nCurrent profit: ${profit}%\nTotal profit: ${totalProfit}%\n\n`,
                err => {
                    if (err) throw err;
...
try {
    fs.appendFile(
        'message.txt',
        `--------------------------------------------\nBot started working at: ${moment().format(
            'MMMM Do YYYY, h:mm:ss a'
        )}\n--------------------------------------------\n`,
        err => {}
    );
