...
const resetValuesAfterSell = (botState, indicatorsData) => {
  ...
  botState.dealType = 'UNDETERMINED';
};
    ...
    if (!botState.strategies[`${strategy}`].stopLoss) {
      try {
        ...
        if (botState.logToTelegram) {
          ...
          ATR 15m: ${indicatorsData.atr.atr15m.prevAtr +
            ' UP: ' +
            indicatorsData.atr.atr15m.buySignalCount +
            ' DOWN: ' +
            indicatorsData.atr.atr15m.downSignalCount}
