...
(async function() {
  ...
  const tradesListStrategy = async pricesStream => {
    const pricesArrLength = pricesStream.length;
    currentPrice = Number(pricesStream[pricesStream.length - 1]);
    const currentAvPrice =
      pricesStream.reduce(sumPricesReducer, 0) / pricesArrLength;
    if (!prevAvPrice) {
      prevAvPrice = currentAvPrice;
      console.log('No prev price found');
      return;
    }
    const avPriceDiff = (currentAvPrice / prevAvPrice) * 100;
    prices.push(currentPrice);
    const profit = buyPrice
      ? currentPrice / buyPrice > 1
        ? Number((currentPrice / buyPrice) * 100 - 100)
    ...
    if (
      !canISell &&
      _isEqual(upSortedPricesArr, pricesStream) &&
      avPriceDiff >= 1.01
    ) {
      canISell = true;
      buyPrice = currentPrice;
    ...
    if (
      canISell &&
      _isEqual(downSortedPricesArr, pricesStream) &&
      avPriceDiff <= 0.99
    ) {
      canISell = false;
      totalProfit += profit;
  ...
  getTradeStream({
    ...
    .pipe(pluck('price'), bufferCount(50, 50))
    .subscribe(tradesListStrategy);
